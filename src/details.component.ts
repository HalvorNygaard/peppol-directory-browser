...
import { PeppolMatch, EntityEntry, Identifier, ContactEntry } from "./peppol.types";
...
export class DetailsComponent {
  ...
  parsedParticipant: { registerName: string; organizationId: string } = {
    registerName: DEFAULTS.UNKNOWN,
    organizationId: DEFAULTS.UNKNOWN,
  };
  readonly TEXT = TEXT;
  ...
  getIdentifiers(entity: EntityEntry | any): Identifier[] {
    ...
    return Array.isArray(raw)
      ? raw.map((i: any) => ({ value: i.value ?? "", scheme: i.scheme }))
      : [];
  }

  /** Return contacts array (defensive) */
  getContacts(entity: EntityEntry | any): ContactEntry[] {
    return Array.isArray(entity?.contact)
      ? entity.contact
      : Array.isArray(entity?.contacts)
        ? entity.contacts
        : [];
  }
  ...
  formatDocDisplay(docId: string): {
    title: string;
    scheme: string;
    subtitle: string;
    originalScheme: string;
    originalCustomization: string;
    ...
    const subtitle = (originalCustomization || "").replace(
      /^(?:urn:)?(?:cen\.eu:en16931:2017#[^#]+#)?(?:urn:)?/i,
      "",
    );
  ...
  parsePeppolDocId(
    docId: string,
  ): { rootSchema: string; rootElement: string; customizationId: string; version: string } | null {
